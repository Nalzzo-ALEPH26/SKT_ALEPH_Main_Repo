...
- 초기 착수는 자기 차례에 빈 교차점 3곳을 먼저 선택한 뒤 `INITIAL LOCK`으로 한 번에 확정합니다.
- 동시 라운드의 15초 타이머와 `LOCK COORDINATES` 버튼은 바둑판 바로 위 고정 컨트롤에 표시됩니다.
- 승패가 결정되면 승자와 게임 종료 상태를 전체 화면 팝업으로 표시합니다.
