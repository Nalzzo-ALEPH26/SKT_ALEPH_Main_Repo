...
await call('draft',B,{round:1,cells:[7,8,9],submit:true});let state=JSON.parse(sql.prepare('SELECT state FROM rooms WHERE code=?').get(A.code).state);state.deadline=Date.now()-1;sql.prepare('UPDATE rooms SET state=? WHERE code=?').run(JSON.stringify(state),A.code);await call('poll',A);
